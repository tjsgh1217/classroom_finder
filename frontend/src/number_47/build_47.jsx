...
const Build47 = () => {
  ...
  const availableRooms = ['470101', '470106'];
  ...
  useEffect(() => {
    const fetchBuildingSchedules = async () => {
      try {
        setLoading(true);
        const response = await API.get(`/courses/building?code=47`);
  ...
  const handleRoomClick = (roomId) => {
    const roomCode = `47${String(selectedFloor).padStart(2, '0')}${String(
      roomId
    ).padStart(2, '0')}`;
    ...
    <div className="building-container">
      ...
      <header className="building-header">
        <div className="header-content">
          <h1>{buildingId}번 건물 - 한남글로벌센터 (무어아트홀)</h1>
        </div>
      </header>
        ...
        {loading ? (
          ...
          <div className="rooms-grid">
            {floorRoomNumbers[selectedFloor]?.map((roomId) => {
              const roomCode = `47${String(selectedFloor).padStart(
                2,
                '0'
