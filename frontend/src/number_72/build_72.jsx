...
const Build72 = () => {
  ...
  const floorRoomNumbers = {
    1: [6, 7, 16, 24, 25, 26, 28, 29, 30, 32, 33, 36, 37, 62],
    2: [8, 13, 25, 26],
  };
  ...
  useEffect(() => {
    const fetchBuildingSchedules = async () => {
      try {
        setLoading(true);
        const response = await API.get(`/courses/building?code=72`);
  ...
  const handleRoomClick = (roomId) => {
    const roomCode = `72${String(selectedFloor).padStart(2, '0')}${String(
      roomId
    ).padStart(2, '0')}`;
        ...
        {loading ? (
          ...
          <div className="rooms-grid">
            {floorRoomNumbers[selectedFloor]?.map((roomId) => {
              const roomCode = `72${String(selectedFloor).padStart(
                2,
                '0'
