...
const Build71 = () => {
  ...
  useEffect(() => {
    const fetchBuildingSchedules = async () => {
      try {
        setLoading(true);
        const response = await API.get(`/courses/building?code=71`);
  ...
  const handleRoomClick = (roomId) => {
    const roomCode = `71${String(selectedFloor).padStart(2, '0')}${String(
      roomId
    ).padStart(2, '0')}`;
        ...
        {loading ? (
          ...
          <div className="rooms-grid">
            {floorRoomNumbers[selectedFloor]?.map((roomId) => {
              const roomCode = `71${String(selectedFloor).padStart(
                2,
                '0'
