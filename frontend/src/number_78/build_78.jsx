...
const Build78 = () => {
  ...
  const availableRooms = ['780112', '780113', '780114'];
  ...
  useEffect(() => {
    const fetchBuildingSchedules = async () => {
      try {
        setLoading(true);
        const response = await API.get(`/courses/building?code=78`);
  ...
  const handleRoomClick = (roomId) => {
    const roomCode = `78${String(selectedFloor).padStart(2, '0')}${String(
      roomId
    ).padStart(2, '0')}`;
    ...
    <div className="building-container">
      ...
      <header className="building-header">
        <div className="header-content">
          <h1>{buildingId}번 건물 - Inno-Biz Park 창업보육센터</h1>
        </div>
      </header>
        ...
        {loading ? (
          ...
          <div className="rooms-grid">
            {floorRoomNumbers[selectedFloor]?.map((roomId) => {
              const roomCode = `78${String(selectedFloor).padStart(
                2,
                '0'
