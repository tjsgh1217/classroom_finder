...
const Build74 = () => {
  ...
  useEffect(() => {
    const fetchBuildingSchedules = async () => {
      try {
        setLoading(true);
        const response = await API.get(`/courses/building?code=74`);
  ...
  const handleRoomClick = (roomId) => {
    const roomCode = `74${String(selectedFloor).padStart(2, '0')}${String(
      roomId
    ).padStart(2, '0')}`;
        ...
        {loading ? (
          ...
          <div className="rooms-grid">
            {floorRoomNumbers[selectedFloor]?.map((roomId) => {
              const roomCode = `74${String(selectedFloor).padStart(
                2,
                '0'
